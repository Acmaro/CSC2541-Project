...
End-to-end pipeline for generating and evaluating synthesizable drug-like molecule analogs using a conditional generative model. Given a source molecule, the pipeline generates structurally similar analogs conditioned on its molecular properties and scores them for similarity, drug-likeness, and novelty.

## Overview

```
ChEMBL download → filter → sample → featurize → generation API → score → results
```
...
| Generate | `sampler.py` | Batch-call generation API for all molecules |
...
The generation API uses a `QuerySampler` with a product-of-experts AND query over all five property types (ECFP4, FCFP4, RDKit descriptors, RDKit upper bound, BRICS fragments) for conditional analog generation.
