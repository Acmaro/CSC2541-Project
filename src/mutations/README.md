# Post-Generation Molecular Modification (Mutations)
...
We categorize mutation algorithms into two main approaches:
...
**CRITICAL:** Different mutation tools have conflicting dependency requirements. Always ensure you activate the correct environment before running a specific tool. Refer to the `envs/` directory in the project root.
...
### The Fingerprint-Mutation Pipeline
...
```bash
# Basic usage: mutate input SMILES toward target fingerprint
python src/mutations/ml_based/pipeline/run.py \
...
python src/mutations/ml_based/pipeline/run.py --input "..." --target "..." --skip-rl

# Adjust RL intensity
python src/mutations/ml_based/pipeline/run.py --input "..." --target "..." --n-steps 500 --batch 128

# Add QED as a secondary objective (weight 0.3 alongside Tanimoto weight 1.0)
python src/mutations/ml_based/pipeline/run.py --input "..." --target "..." --qed 0.3
...
If you need to interact directly with the Lib-INVENT core (run from inside `src/mutations/ml_based/Lib-INVENT/`):
